    ...
    <ul id="list" class="mui-table-view mui-table-view-chevron">
      <li class="mui-table-view-cell mui-media" v-for="item in Res" :key="item.id">
        <router-link :to="'/home/musinfo/'+item.id">
          <img
            class="mui-media-object mui-pull-left cirImg"
            data-lazyload-id="0"
            :src="item.img1v1Url"
          >
      ...
      if (this.searchInfo != "") {
        this.$http
          ...
          .then(result => {
            // console.log(result);
            if (this.countFlag) {
              this.count = result.body.result.artistCount;
              this.countFlag = false;
            }
            if (
              result.status === 200 &&
              result.body.result.artists != undefined
            ) {
              // &&result.body.result.albums!=undefined
              this.Res = result.body.result.artists;
